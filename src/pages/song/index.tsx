import { ColumnsType } from 'antd/es/table';
import { useBulkDeleteSong, useGetSong } from 'api/company/useSong';
import { SelectStatus } from 'components/atomic/common/Status';
import { InputSearchGlobal } from 'components/custom/Input/InputSearchGlobal';
import TableGlobal, { ActionsRow, HeaderTable } from 'components/custom/Table';
import { useSearchFilters, useSelectTableRow } from 'hooks';
import { ScreenType } from 'types';
import { SongRes } from 'types/SwaggerTypeUser';
import { openModal } from 'store/slices/modalSlice';
import { Key } from 'antd/es/table/interface';
import openModalConfirm from 'components/Modal/ModalConfirm';
import { useTranslation } from 'react-i18next';
import { ModalAddUpdateSong } from './components/ModalAddUpdateSong';

const screen: ScreenType = 'SONG';

const Song = () => {
  const { t } = useTranslation();
  const { handleSelectRow, selectedKey, setSelectedKey } = useSelectTableRow();
  const { params, setParams, setSearchValue, handleChangePagination } = useSearchFilters({
    page: 0,
  });
  const { data: song, isFetching } = useGetSong(params);
  const { mutateAsync: deleteSong } = useBulkDeleteSong();

  const columns: ColumnsType<SongRes> = [
    {
      title: t('code'),
      dataIndex: 'code',
      ellipsis: true,
    },
    {
      title: t('song_name'),
      dataIndex: 'name',
      ellipsis: true,
    },
    {
      title: t('singer'),
      dataIndex: 'singer',
      ellipsis: true,
    },
    {
      title: t('author'),
      dataIndex: 'author',
      ellipsis: true,
    },

    {
      width: 200,
      fixed: 'right',
      render: (_, record) => (
        <ActionsRow
          onDelete={() => handleBulkDelete(record.id)}
          onUpdate={() => handleCreateUpdate(record)}
          screen={screen}
        />
      ),
    },
  ];

  const handleCreateUpdate = (song?: SongRes) => {
    openModal({
      content: <ModalAddUpdateSong songId={song?.id ?? 0} screen={screen} />,
      options: {
        title: t('view_song'),
        width: 800,
      },
    });
  };

  const handleBulkDelete = (id?: Key) => {
    openModalConfirm({
      modalType: 'delete',
      onOk: () =>
        deleteSong(
          {
            ids: id ? [id] : selectedKey,
          },
          {
            onSuccess: () => setSelectedKey([]),
          },
        ),
    });
  };

  return (
    <div>
      <HeaderTable
        title={t('song')}
        onCreate={() => handleCreateUpdate()}
        onBulkDelete={handleBulkDelete}
        disableDelete={!selectedKey.length}
        screen={screen}
      />
      <div className="my-2 flex gap-2">
        <SelectStatus
          className="w-80"
          onChange={(value) => setParams((pre) => ({ ...pre, status: value }))}
          allowClear
        />
        <InputSearchGlobal onChange={(e) => setSearchValue(e.target.value)} tooltip={t('song_name')} />
      </div>
      <TableGlobal
        columns={columns}
        dataSource={song?.data}
        onChange={handleChangePagination}
        rowSelection={{ onChange: handleSelectRow }}
        loading={isFetching}
        totalItems={song?.total_record}
        currentPage={params.page}
      />
    </div>
  );
};

export default Song;
